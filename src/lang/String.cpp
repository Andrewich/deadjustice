 ...
 */
template <class T>
static int arrayLength(const T* str) {
  const T* str0 = str;
  while (*str) ++str;
  int len = str - str0;
  return len;
...
/**
 * Copies specified number of elements from source Char array to the destination
 * Char array. Terminates destination Char array with 0.
 */
...
static Char* createString(int maxLen, void* buffer, int bufferSize) {
  assert(bufferSize > sizeof(StringRep) +
                          sizeof(Char) * 4);  // Minimum size of string buffer
...
inline static void addStringReference(Char* str) {
  assert(isStringDynamic(
      str));  // we can't reference automatically allocated strings
...
String::String() { m_this = createString(0, m_buffer, sizeof(m_buffer)); }
...
String::~String() { releaseString(m_this); }
...
int String::length() const { return getStringSize(m_this); }
...
void String::getChars(int begin, int end, Char* dest) const {
  ...
  for (; begin < end; ++begin) *dest++ = m_this[begin];
}
...
bool String::endsWith(const String& suffix) const {
  ...
  if (otherLen <= thisLen)
    return 0 ==
           arrayCompare(m_this + thisLen - otherLen, suffix.m_this, otherLen);
  else
    return false;
}
...
int String::indexOf(Char ch, int index) const {
  ...
  for (; index < thisLen; ++index) {
    if (m_this[index] == ch) return index;
  }
  return -1;
}
...
int String::indexOf(const String& str, int index) const {
  ...
  for (; index <= lastIndex; ++index) {
    int j = 0;
    for (; j < strLen; ++j) {
      if (str.m_this[j] != m_this[index + j]) break;
    }
    if (strLen == j) return index;
  }
  return -1;
}
...
int String::lastIndexOf(const String& str, int index) const {
  ...
  if (index + strLen > thisLen) index = thisLen - strLen;
...
bool String::regionMatches(int thisOffset, const String& other, int otherOffset,
                           int length) const {
  int thisLen = this->length();
  int otherLen = other.length();

  if (thisOffset >= 0 && otherOffset >= 0 && thisOffset + length <= thisLen &&
      otherOffset + length <= otherLen) {
    for (int c = 0; c < length; ++c) {
      if (m_this[thisOffset + c] != other.m_this[otherOffset + c]) return false;
    }
    return true;
  }
  return false;
...
String String::toLowerCase() const {
  ...
  for (int i = 0; i < thisLen; ++i) {
    Char ch = m_this[i];
    if (ch < Char(0x80)) ch = (Char)tolower((char)ch);
    s.m_this[i] = ch;
  }
...
String String::toUpperCase() const {
  ...
  for (int i = 0; i < thisLen; ++i) {
    Char ch = m_this[i];
    if (ch < Char(0x80)) ch = (Char)toupper((char)ch);
    s.m_this[i] = ch;
  }
...
String String::trim() const {
  ...
  for (; begin < thisLen; ++begin) {
    ...
    if (ch >= Char(0x80) || !isspace((char)ch)) break;
  }
  for (; end > 0; --end) {
