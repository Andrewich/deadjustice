//
// hemisphere diffuse bump + specular*gloss /w point light
//
...
// DJ constants
// float3			SKY_COLOR_MIN = float3( 23/255.0, 23.0/255.0, 23.0/255.0 );
// float3			GROUND_COLOR_MIN = float3( 12/255.0, 12/255.0, 12/255.0 );
// float3			SPECULAR_COLOR_MIN = float3( 0.035, 0.035, 0.045 );

// float3			SKY_COLOR_MAX = float3( 230/255.0, 230.0/255.0, 235.0/255.0 );
// float3			GROUND_COLOR_MAX = float3( 120/255.0, 120/255.0, 125/255.0 );
// float3			SPECULAR_COLOR_MAX = float3( 0.35, 0.35, 0.45 );

float3			SKY_COLOR_MIN = float3( 23.0/255.0, 23.0/255.0, 27.0/255.0 );
float3			GROUND_COLOR_MIN = float3( 12.0/255.0, 12.0/255.0, 15.0/255.0 );
float3			SPECULAR_COLOR_MIN = float3( 0.035, 0.035, 0.045 );

float3			SKY_COLOR_MAX = float3( 245.0/255.0, 245.0/255.0, 255.0/255.0 );
float3			GROUND_COLOR_MAX = float3( 120.0/255.0, 120.0/255.0, 125.0/255.0 );
float3			SPECULAR_COLOR_MAX = float3( 0.35, 0.35, 0.45 );

// constants
float3			SKY_COLOR = float3( 230/255.0, 230.0/255.0, 235.0/255.0 );
float3			GROUND_COLOR = float3( 120/255.0, 120/255.0, 125/255.0 );
float3			SPECULAR_COLOR = float3( 0.35, 0.35, 0.45 );
...
{
	float3 gloss = SPECULAR_COLOR * tex2D( diffuseSampler, p.tc0 ).w;
	float3 normv = ( tex2D( bumpSampler, p.tc1 ) - 0.5 ) * 2;
	float3 halfv = ( p.halfv - 0.5 ) * 2;
...
technique T0
{
	...
	pass P1
	{
		VertexShader = compile vs_1_1 bumpSpecularVS();
